...
- All three sites sit behind Cloudflare and hard-block non-browser clients (403). Each core therefore ships the same mechanism: `isHardBlock` short-circuits mirror loops, `fetchCodeByWebView` fallback (shares kernel/UA/CookieManager with the embedded verification page), a `<app>.webviewMode` flag set by the verification page's step-2 button that routes requests webview-first, hard-blocked failures cached ~30s, and a conditional "验证并同步" entry shown only when actually blocked. Never try to extract/replay cookies across contexts — X5's jar is isolated from both the system CookieManager and rule-scope vars.
