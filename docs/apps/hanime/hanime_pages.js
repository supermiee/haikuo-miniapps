...
(function () {
    ...
    function emptyRule(method, params, source) {
        return $('hiker://empty' + (source ? '#' + source : '')).rule(function (payload) {
            var pages;
            try { pages = $.require('https://supermiee.github.io/haikuo-miniapps/apps/hanime/hanime_pages.js?v=11'); } catch (ignore) {}
    ...
    function renderHome() {
        ...
        var result = [{ title: '搜索 Hanime1', desc: '输入标题或作者', url: "input ? (function(){return $.require('https://supermiee.github.io/haikuo-miniapps/apps/hanime/hanime_pages.js?v=11').routeSearch(input);})() : 'toast://请输入关键词'", col_type: 'input', extra: { defaultValue: '' } }];
    ...
    function renderVerification() {
        ...
        setResult([
            ...
            { title: '第二步：验证成功后，点此返回并刷新', url: $('hiker://empty').lazyRule(function () {
                /* 规则间的 putVar 是可靠的（不可靠的只是网页注入桥）；置位后请求层自动改走与验证网页同源的 WebView 通道 */
                try { putVar('hanime1.webviewMode', '1'); } catch (ignoreFlag) {}
                try { $.require('https://supermiee.github.io/haikuo-miniapps/apps/hanime/hanime_core.js?v=11').clearPageCache(); } catch (ignoreClear) {}
                back(true);
            ...
            { title: '备用：粘贴 cf_clearance', desc: '仅在内嵌网页始终无法通过时使用。粘贴后自动保存并返回刷新。', url: "$.require('https://supermiee.github.io/haikuo-miniapps/apps/hanime/hanime_pages.js?v=11').importCookie(input)", col_type: 'input', extra: { defaultValue: '' } }
        ]);
    }
